...
$inputPost = filter_input_array(INPUT_POST, FILTER_VALIDATE_INT);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if(!$idBeneficiario) {
            ...
            header("Location: tab-deficiente");
            exit();
        }

        if ($deleted){
            $_SESSION['alert-beneficiario'] = setAlert("Cartão excluido com sucesso!", "success");
        } else {
            $_SESSION['alert-beneficiario'] = setAlert("Erro ao excluir o cartão", "error");
        }
