...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $deleted = $formDeficienteModel->deleteAllFormDeficiente();

    if($deleted) {
        $_SESSION['alert-beneficiario'] = setAlert("Todos os formulários de deficientes foram deletados com sucesso!", "success");
        header('Location: tab-deficiente.php');
        exit();
    } else {
        $_SESSION['alert-beneficiario'] = setAlert("Erro ao deletar formulários de deficientes!", "error");
        header('Location: tab-deficiente.php');
        exit();
    }
}