...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($newsModel->getNewsById($id)['destaque'] == intval(0)) {
        ...
        if($result) {
            ...
            $_SESSION['news-alert'] = setAlert("Erro ao destacar notícia", "error");
        }
    } else {
        $result = $newsModel->unfeatureNews($id);
        if($result) {
            $_SESSION['news-alert'] = setAlert("Destaque removido com sucesso!", "success");
        } else {
            $_SESSION['news-alert'] = setAlert("Erro ao remover destaque", "error");
        }
    }
