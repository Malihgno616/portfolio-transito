...
$idNotificacao = filter_input(INPUT_POST, 'id-notificacao', FILTER_VALIDATE_INT);
    
if($_SERVER['REQUEST_METHOD'] === 'POST') {

    try {      
        $deletedNotification = $notificacaoModel->deleteNotification($idNotificacao);

        if ($deletedNotification) {
            $_SESSION['notificacao-alert'] = setAlert("Notificação excluída com sucesso!", 'success');
        } else {
            $_SESSION['notificacao-alert'] = setAlert("Falha ao excluir a notificação", 'error');
        }

    } catch (Exception $e) {
        $_SESSION['notificacao-alert'] = setAlert("Erro ao excluir a notificação: " . $e->getMessage(), 'error');
    }
    ...
    $_SESSION['notificacao-alert'] = setAlert("Acesso não autorizado", 'error');
    header("Location: notificacoes.php");
    exit();
}