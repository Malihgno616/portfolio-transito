...
$inputPost = filter_input_array(INPUT_POST, FILTER_SANITIZE_SPECIAL_CHARS);

$idContent = $inputPost['id-content-unhighlight'] ?? null;
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
   try {
        ...
        $newsModel->unhighlightNews($idContent);

        $_SESSION['news-alert'] = setAlert("Destaque removido!", 'success');
        header("Location: news-table.php");
        exit();
   } catch (Exception $e) {
        $_SESSION['news-alert'] = setAlert("Erro ao remover destaque: " . $e->getMessage(), 'error');
        header("Location: news-table.php");
        exit();
   }
}