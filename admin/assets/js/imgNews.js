document.getElementById('dropzone-file-main-news').addEventListener('change', function(e) {
  ...
  const imageInput = document.querySelector('input[name="name-img-file-main"]'); // Seleciona o input de nome da imagem
...
document.getElementById('dropzone-file-content-news').addEventListener('change', function(e) {
  const file = e.target.files[0];
  const preview = document.getElementById('image-preview-content');
  const dropzone = this.closest('label'); // Pega o elemento label que contém todo o dropzone
  const imageInput = document.querySelector('input[name="name-img-file-content"]'); // Seleciona o input de nome da imagem
  const textElements = dropzone.querySelectorAll('svg, p'); // Seleciona o ícone e os textos
