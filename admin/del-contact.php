...
if($reqMethod === 'POST') {
  ...
  if(!$id) {
    $_SESSION['error_message'] = setAlert("ID inválido!");
    header('Location: contatos.php');
    exit();
  }

  try {
    $deleted = $model->deleteContact($id);   
    
    if($deleted) {
      $_SESSION['success_del_contact'] = setAlert("Informação deletada com sucesso!", "success");
      header('Location: contatos.php');
      exit();

    } else {
      $_SESSION['error_message'] = setAlert("Erro ao deletar informação!", 'error');
      header('Location: contatos.php');
      exit();
    }

  } catch(Exception $e) {
    $_SESSION['error_message'] = setAlert($e->getMessage(), 'error');
    header('Location: contatos.php');
    exit();
  }
