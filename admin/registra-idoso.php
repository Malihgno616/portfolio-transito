...
$numRegistro = $inputPost['numero-registro-idoso'];
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $updated = $formIdosoModel->addRegistration($idIdoso, $numRegistro, $date);

    if($updated) {
        $_SESSION['idoso-alert'] = setAlert("Nº de registro do idoso adicionado com sucesso!", 'success');
    } else {
        $_SESSION['idoso-alert'] = setAlert("Erro ao adicionar nº de registro do idoso.", 'error');
    }
    ...
    exit;
        ...
        $_SESSION['idoso-alert'] = setAlert("Erro ao adicionar nº de registro do idoso: " . $e->getMessage(), 'error');
        header("Location: tab-idoso.php");
        exit;
    }
