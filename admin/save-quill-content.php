...
function setAlert($message, $type = 'success') {
  ...
  if($_SERVER['REQUEST_METHOD'] === 'POST') {
    $idTela = $_POST['id-tela'];
    $conteudo = $_POST['conteudo'] ?? null;

    try {
        $paginaExistente = $telaSiteModel->getContentById($idTela);
        
        if(empty($paginaExistente)) {
            $telaSiteModel->addContentPage($idTela, $conteudo);
        } else {
            $telaSiteModel->editContentPage($idTela, $conteudo);
        }
        $_SESSION['content-alert'] = setAlert("Conteúdo salvo com sucesso!", "success");
        
    } catch(Exception $e) {
    $_SESSION['content-alert'] = setAlert("Erro ao salvar o conteúdo: ". $e->getMessage(), "error");
    }

    header("Location: tela.php?tela=".$idTela);
    exit;
