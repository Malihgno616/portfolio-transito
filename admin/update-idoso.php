...
// Flags para controle de atualização de arquivos
$updateRgIdoso = false;
$updateRgRep = false;
$updateCompRep = false;
...
if (isset($_FILES['copia-rg-idoso']) && $_FILES['copia-rg-idoso']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($mimeTypeIdoso, ['image/jpeg', 'image/png', 'application/pdf'])) {
        $_SESSION['idoso-alert'] = setAlert('Tipo de arquivo inválido para RG do idoso. Apenas JPEG, PNG e PDF são permitidos.', 'error');
        header('Location: tab-idoso.php');
        exit;
    }

    $copiaRgIdoso = $fileContentIdoso;  
    $nomeAqvRgIdoso = $_FILES['copia-rg-idoso']['name'];
    $updateRgIdoso = true;
} else {
    $copiaRgIdoso = null;
...
if (isset($_FILES['copia-rg-rep']) && $_FILES['copia-rg-rep']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($mimeTypeRep, ['image/jpeg', 'image/png', 'application/pdf'])) {
        $_SESSION['idoso-alert'] = setAlert('Tipo de arquivo inválido para RG do representante. Apenas JPEG, PNG e PDF são permitidos.', 'error');
        header('Location: tab-idoso.php');
        exit;
    }

    $copiaRgRep = $fileContentRep;  
    $nomeAqvRgRep = $_FILES['copia-rg-rep']['name'];
    $updateRgRep = true;
} else {
    $copiaRgRep = null;
...
if (isset($_FILES['comprovante-rep']) && $_FILES['comprovante-rep']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($mimeTypeCompRep, ['image/jpeg', 'image/png', 'application/pdf'])) {
        $_SESSION['idoso-alert'] = setAlert('Tipo de arquivo inválido para comprovante. Apenas JPEG, PNG e PDF são permitidos.', 'error');
        header('Location: tab-idoso.php');
        exit;
    }

    $comprovanteRep = $fileContentCompRep;
    $nomeAqvCompRep = $_FILES['comprovante-rep']['name'];
    $updateCompRep = true;
} else {
    $comprovanteRep = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($updateSuccess) {
        $_SESSION['idoso-alert'] = setAlert('Dados do idoso atualizados com sucesso!', 'success');
    } else {
        $_SESSION['idoso-alert'] = setAlert('Erro ao atualizar os dados do idoso. Por favor, tente novamente.', 'error');
    }
