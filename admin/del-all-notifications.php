...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $deleted = $notificacaoModel->deleteAllNotifications();

    if($deleted) {
        $_SESSION['notificacao-alert'] = setAlert("Todas as notificações foram deletadas com sucesso!", "success");
        header('Location: notificacoes.php');
        exit();
    } else {
        $_SESSION['notificacao-alert'] = setAlert("Erro ao deletar notificações!", "error");
        header('Location: notificacoes.php');
        exit();
    }
}
