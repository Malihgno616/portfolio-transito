<div class="shadow-lg bg-stone-500 bg-blend-multiply p-10 bg-no-repeat bg-cover bg-fixed px-4 md:px-6 lg:px-8" style="background-image: url('assets/img/imgtransito-compressed.jpg');">
  ...
  <div class="max-w-7xl mx-auto">
    ...
    <?php if(is_array($noticiasDestaque) && !empty($noticiasDestaque)): ?>
      <?php foreach ($noticiasDestaque as $index => $newsCards): ?>
      <?php
  ...
  <div class="max-w-7xl mx-auto">
    ...
    <?php if(is_array($noticiasRecentes) && !empty($noticiasRecentes)): ?>
      <?php foreach ($noticiasRecentes as $index => $newsCards): ?>
      <?php
        $delays = ['animate__delay-0s', 'animate__delay-1s', 'animate__delay-2s'];
        $delayClass = $delays[$index % count($delays)] ?? 'animate__delay-0s';
      ?>
