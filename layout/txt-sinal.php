<main class="texto-format animate__animated animate__fadeIn mx-auto p-5 sm:p-10">

  <h1 class="text-4xl sm:text-5xl p-5">Sinalizações</h1>
  <p class="text-justify text-lg sm:text-xl p-5">Nesta página estão relacionadas as solicitações de sinalizações (placas
    de regulamentação e/ou advertências, entre outras).</p>

  <section class="flex flex-wrap sm:flex-col items-start gap-4 mb-7">
    <p class="text-base sm:text-sm px-5 sm:px-0 sm:text-center">
      Para solicitação de qualquer tipo de sinalização, nos encaminhe para este e-mail:
    </p>
    <a class="bg-yellow-500 ml-4 px-4 py-2 rounded-md hover:bg-amber-200 duration-200 text-lg" href="#">
      nucleodetransito@leme.sp.gov.br
    </a>
  </section>

  <section class="flex flex-wrap sm:flex-col items-start gap-4 mb-7">
    <p class="text-base sm:text-sm p-5 w-full sm:w-auto">Ou entre em contato conosco através do nosso formulário de
      contato.</p>
    <a class="bg-yellow-500 ml-4 px-4 py-2 rounded-md hover:bg-amber-200 duration-200 text-lg"
      href="contato" title="Formulário de Contato">
      Formulário de Contato
    </a>
  </section>

</main>