...
$erro_sql = isset($_SESSION['erro-sql']) ? $_SESSION['erro-sql'] : null ;

unset($_SESSION['sucesso']);
unset($_SESSION['erro']);
unset($_SESSION['erro-sql']);
unset($_SESSION['erro-campos']);
...
<form class="max-w-200 mx-auto m-20 p-5 border-2 border-gray-200 rounded-md animate__animated animate__fadeIn" action="../config/database/form-contato-db.php" method="post">
  ...
  <?php if (!empty($erro_campo['mensagem'])): ?>
    ...
    <div class="relative mb-5">
      ...
      <label for="mensagem" class="absolute text-xl text-gray-500 peer-focus:text-yellow-500 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Digite sua mensagem...</label>
    </div>
  <?php endif; ?>
