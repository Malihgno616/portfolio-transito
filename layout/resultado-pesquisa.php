<div class="p-10">
    <div class="mb-6">
        ...
        <form action="pesquisa-noticia" class="flex items-center justify-center gap-2 w-full md:w-auto" method="get">
            <input type="search" name="term" id="term" placeholder="Buscar notícia..." class="w-full md:w-64 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" value="<?= htmlspecialchars($term)?>"/>
            <button type="submit" class="px-4 py-2 bg-yellow-500 text-gray-900 font-medium rounded-lg hover:bg-yellow-600 duration-200 whitespace-nowrap cursor-pointer"><i class="fas fa-search"></i></button>
        </form>
        <h2 class="text-black text-center text-2xl md:text-2xl lg:text-4xl p-5">
            Resultados de: <span class="text-yellow-500"><?= htmlspecialchars($term) ?></span>
        </h2>
    </div>
